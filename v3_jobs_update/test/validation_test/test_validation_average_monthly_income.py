...
from v3_jobs_update.schemas_validation.schemas_average_monthly_income import request_average_monthly_income


def extract_validation_error(response_json):
    for group in response_json.get("result", {}).get("componentGroups", []):
        for component in group.get("components", []):
            if component.get("label") == "Среднемесячная зарплата":
...
@allure.title("Тест на валидацию Среднемесячного дохода: brokerId = {broker_id}, biztalkId = {biztalk_id}")
@allure.description("Проверка срабатывания валидации на бэке для поля AverageMonthlyIncome")
@pytest.mark.parametrize("averageMonthlyIncome, expected_error", [
    ("0", "Доход должен быть больше нуля"),
    ("-1", "Доход должен быть больше нуля"),
])

def test_average_monthly_income_validation(averageMonthlyIncome, expected_error):
    url = f"{Config.BASE_URL_PREAPPROVED}/{VALID_BROKER_ID}/jobs/{job_id}/update/{VALID_BIZTALK_ID}"
    payload = request_average_monthly_income(averageMonthlyIncome)
    ...
    with allure.step(f"Для поля AverageMonthlyIncome сработала валидация. ValidationError = {validation_error}"):
