...
from v3_personal_data_update_bdf_screen.schemas_validation.schemas_actual_address import request_body_registration_address


def extract_validation_error(response_json):
    for group in response_json.get("result", {}).get("componentGroups", []):
        for component in group.get("components", []):
            if component.get("label") == "Адрес проживания":
...
@allure.title("Тест на валидацию Актуального адреса: brokerId = {broker_id}, biztalkId = {biztalk_id}")
@allure.description("Проверка срабатывания валидации на бэке для поля Адрес проживания")
@pytest.mark.parametrize("actualAddress, expected_error", [
...
def test_actual_address_validation(actualAddress, expected_error):
    url = f"{Config.BASE_URL_PREAPPROVED}/{VALID_BROKER_ID}/personal-data/update-bdf-screen/{VALID_BIZTALK_ID}"
    payload = request_body_registration_address(actualAddress)
    ...
    with allure.step(f"Для поля ActualAdress сработала валидация. ValidationError = {validation_error}"):
