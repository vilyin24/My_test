...
from v3_personal_data_update_bdf_screen.schemas_validation.schemas_education import request_body_education


def extract_validation_error(response_json):
    for group in response_json.get("result", {}).get("componentGroups", []):
        for component in group.get("components", []):
            if component.get("label") == "Уровень образования":
...
@allure.title("Тест на валидацию Образования: brokerId = {broker_id}, biztalkId = {biztalk_id}")
@allure.description("Проверка срабатывания валидации на бэке для поля Education")
@pytest.mark.parametrize("education, expected_error", [
    ("null", "Необходимо выбрать значение"),
    (" ", "Необходимо выбрать значение"),
    ("", "Необходимо выбрать значение"),
])

def test_education_validation(education, expected_error):
    url = f"{Config.BASE_URL_PREAPPROVED}/{VALID_BROKER_ID}/personal-data/update-bdf-screen/{VALID_BIZTALK_ID}"
    payload = request_body_education(education)
    ...
    with allure.step(f"Для поля Education сработала валидация. ValidationError = {validation_error}"):
