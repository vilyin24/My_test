...
from v3_jobs_create.schemas_validation.schemas_company_name import request_company_name


def extract_validation_error(response_json):
    for group in response_json.get("result", {}).get("componentGroups", []):
        for component in group.get("components", []):
            if component.get("label") == "Наименование организации":
...
@allure.title("Тест на валидацию Наименования организации: brokerId = {broker_id}, biztalkId = {biztalk_id}")
@allure.description("Проверка срабатывания валидации на бэке для поля CompanyName")
@pytest.mark.parametrize("companyName, expected_error", [
    ("", "Название организации не может быть пустым"),
])

def test_company_name_validation(companyName, expected_error):
    url = f"{Config.BASE_URL_PREAPPROVED}/{VALID_BROKER_ID}/jobs/create/{VALID_BIZTALK_ID}"
    payload = request_company_name(companyName)
    ...
    with allure.step(f"Для поля CompanyName сработала валидация. ValidationError = {validation_error}"):
