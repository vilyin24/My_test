...
from v3_jobs_create.schemas_validation.schemas_inn import request_inn
...
@allure.title("Тест на валидацию ИНН: brokerId = {broker_id}, biztalkId = {biztalk_id}")
@allure.description("Проверка срабатывания валидации на бэке для поля INN")
@pytest.mark.parametrize("inn, expected_error", [
    ("", "ИНН не может быть пустым"),
])

def test_inn_validation(inn, expected_error):
    url = f"{Config.BASE_URL_PREAPPROVED}/{VALID_BROKER_ID}/jobs/create/{VALID_BIZTALK_ID}"
    payload = request_inn(inn)
    ...
    with allure.step(f"Для поля INN сработала валидация. ValidationError = {validation_error}"):
