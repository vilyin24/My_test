...
@allure.step("GetCreditPrograms GET-запрос")
def get_programs(biztalk_id):
    url = f"{Config.BASE_URL_CREDIT}/GetCreditPrograms/{biztalk_id}"
    response = requests.get(url, verify=False)
    return response

@allure.feature("GET GetCreditPrograms")
@allure.title("Получения информации о доступном диапазоне суммы кредита и ставке")
@allure.description("GetCreditPrograms GET-запрос Получения информации о доступном диапазоне суммы кредита и ставке по biztalkId")
...
def test_get_jobs(biztalk_id, expected_success, expected_status):
    ...
    if not expected_success:
        with allure.step("Собщение об ошибке"):
